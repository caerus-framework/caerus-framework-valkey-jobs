...
func TestWithConfigOverridesOptions(t *testing.T) {
	j := New(
		...
		WithBatchSize(99),
		WithConcurrency(32),
		WithConfig(JobsConfig{PollIntervalMs: 250, BatchSize: 4, Concurrency: 2, RetryJitter: 0}),
	)
...
func TestInitRequiresValkeyInitialized(t *testing.T) {
	...
	j := New()
	err := j.Init(context.Background(), fw)
	if err == nil {
...
func TestRetryPolicyFixedPhase(t *testing.T) {
	j := New(WithConfig(JobsConfig{RetryJitter: 0}))
	now := time.Now()
