...
// applyConfig overlays non-zero fields of cfg onto the component's settings.
// It runs last, so a loaded config wins over option-set defaults. Callers must
// not hold the mutex.
...
func (c *CFValkeyJobs) Init(ctx context.Context, fw *cf.CaerusFramework) error {
	...
	if !ok {
		return fmt.Errorf("cf_valkey_jobs: valkey component %q is not registered (add it to the framework and to GetDependencies)", c.peerName())
	}
	if vk.Client() == nil {
		return fmt.Errorf("cf_valkey_jobs: valkey component %q is not initialized", c.peerName())
	}
	c.vk = vk
	return nil
}
...
// Run implements cf.Runnable. It runs the worker poll loop until ctx is
// canceled, then drains claimed jobs back to the ready queue and returns
// promptly. No-op when no handlers are registered or worker_enabled is false.
...
func (c *CFValkeyJobs) reapOne(ctx context.Context, client valkey.Client, id string, nowMs int64) {
	status, jtype, err := c.release(ctx, client, id, nowMs, nowMs, 0, 0, true)
	if err != nil {
...
func (c *CFValkeyJobs) processOne(ctx context.Context, client valkey.Client, id string) {
	...
	c.trackInflight(id)
	c.dispatch(ctx, client, job, retryStart, jitterBase, sem)
}

func (c *CFValkeyJobs) dispatch(ctx context.Context, client valkey.Client, job Job, retryStart, jitterBase int64, sem chan struct{}) {
	c.mu.RLock()
	handler, ok := c.handlers[job.Type]
	c.mu.RUnlock()
	if !ok {
		// No handler for this type: dead-letter immediately rather than hot-loop.
		status, err := c.dead(ctx, client, job.ID)
		if err != nil {
	...
	go func() {
		...
		m.durCount.Add(1)
		if err == nil {
			if ackErr := c.ack(ctx, client, job.ID); ackErr != nil {
		...
		status, _, relErr := c.release(ctx, client, job.ID, due, now.UnixMilli(), newStart, newBase, false)
		if relErr != nil {
...
// ack removes a finished job (inflight entry + payload).
func (c *CFValkeyJobs) ack(ctx context.Context, client valkey.Client, id string) error {
...
func (c *CFValkeyJobs) dead(ctx context.Context, client valkey.Client, id string) (int, error) {
...
// the job type.
func (c *CFValkeyJobs) release(ctx context.Context, client valkey.Client, id string, dueMs, deadAtMs, retryStartMs, jitterBase int64, bump bool) (int, string, error) {
	bumpS := "0"
	if bump {
		bumpS = "1"
	}
...
func (c *CFValkeyJobs) drain(ctx context.Context) {
	...
	nowMs := time.Now().UnixMilli()
	for _, id := range ids {
		status, jtype, err := c.release(dctx, client, id, nowMs, nowMs, 0, 0, false)
		if err != nil {
...
func (c *CFValkeyJobs) metricRows(types []string) []cf_observability.Metric {
	...
	for _, t := range types {
		...
		ms = append(ms,
			cf_observability.Metric{Name: "valkey_jobs_enqueued_total", Help: "Total number of jobs enqueued.", Value: float64(m.enqueued.Load()), Labels: copyLabels(lbl), Type: cf_observability.MetricTypeCounter},
			cf_observability.Metric{Name: "valkey_jobs_run_total", Help: "Total number of handler runs.", Value: float64(m.run.Load()), Labels: copyLabels(lbl), Type: cf_observability.MetricTypeCounter},
			cf_observability.Metric{Name: "valkey_jobs_failed_total", Help: "Total number of handler failures.", Value: float64(m.failed.Load()), Labels: copyLabels(lbl), Type: cf_observability.MetricTypeCounter},
			cf_observability.Metric{Name: "valkey_jobs_requeued_total", Help: "Total number of requeues (retries and recovery).", Value: float64(m.requeued.Load()), Labels: copyLabels(lbl), Type: cf_observability.MetricTypeCounter},
			cf_observability.Metric{Name: "valkey_jobs_dead_total", Help: "Total number of dead-lettered jobs.", Value: float64(m.dead.Load()), Labels: copyLabels(lbl), Type: cf_observability.MetricTypeCounter},
			cf_observability.Metric{Name: "valkey_jobs_duration_seconds_sum", Help: "Total handler duration.", Value: float64(m.durSumNs.Load()) / 1e9, Labels: copyLabels(lbl), Type: cf_observability.MetricTypeCounter},
			cf_observability.Metric{Name: "valkey_jobs_duration_seconds_count", Help: "Number of handler duration samples.", Value: float64(m.durCount.Load()), Labels: copyLabels(lbl), Type: cf_observability.MetricTypeCounter},
		)
	}
	return ms
}
...
const luaEnqueue = `redis.call("HSET", KEYS[1], "type", ARGV[1], "payload", ARGV[2],
